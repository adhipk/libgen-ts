...
import { forFields, dups } from "./clean.js"
import got from "got"
import { id } from "./latest.js"
...
export async function text(options) {
  ...
  try {
    const data = await id(options.mirror)
      ...
      try {
        ...
        if (filters && Array.isArray(filters)) {
          ...
          while (f--) {
            ...
            if (cleaned)
              cleaned = forFields(cleaned, filters[f])
          }
          if (cleaned)
            texts = texts.concat(cleaned)

        } else if (filters) {
          cleaned = forFields(raw, filters)
          if (cleaned)
            texts = texts.concat(cleaned)
